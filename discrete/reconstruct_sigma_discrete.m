...
% Boundary (r>=.95) value is much smaller than the real one!
% Don't know why!
% Use the linear interpolation to reconstruct from internal data.
indBou = find(x.^2+y.^2>=.95^2);
indInt = setdiff(1:lenDat,indBou);
funGraX = scatteredInterpolant(x(indInt),y(indInt),graLnSig(indInt,1),'natural','linear');
funGraY = scatteredInterpolant(x(indInt),y(indInt),graLnSig(indInt,2),'natural','linear');
graLnSig(indBou,1) = funGraX(x(indBou),y(indBou));
graLnSig(indBou,2) = funGraY(x(indBou),y(indBou));
...
staX    = cos(staAng);
staY    = sin(staAng);
staDif  = config_coefficient_discrete(staX,staY);
...
difBou = config_coefficient_discrete(x(indBou),y(indBou));
...
D      = config_coefficient_discrete(x,y);
...
% Boundary (r>=.97) value is not correct!
% Assuming constant coefficients near the boundary, then we can correct with 
% \Delta\sqrt(\sigma) / \sqrt(\sigma)) = \mu / D
indBou = find(x.^2+y.^2>=.95^2);
[difBou,absBou] = config_coefficient_discrete(x(indBou),y(indBou));
coeD(indBou)    = absBou./difBou;
coeD= trismooth(x,y,coeD,pre/2);
% verify with background truth
coeDTru= trilaplace(tri,x,y,sqrt(sigTru))./sqrt(sigTru);
indBou = find(x.^2+y.^2>=.95^2);
[difBou,absBou] = config_coefficient_discrete(x(indBou),y(indBou));
coeDTru(indBou) = absBou./difBou;
coeDTru= trismooth(x,y,coeDTru,pre/2);
...
save coefficient_D_discrete x y tri ref H pre sigma sigTru coeD coeDTru D1 dxD1 lapD1 graLnSig
