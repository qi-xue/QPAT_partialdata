...
[difBou,absBou] = config_coefficient_smooth(xx(indBou),yy(indBou));
mu(indBou) = absBou;
figure
...
disp(norm(D(indNor)-DTru(indNor))/norm(DTru(indNor))) % 1.09%
disp("Relative error for absorption coefficient:")
disp(norm(mu(indNor)-muTru(indNor))/norm(muTru(indNor))) % 1.60%
